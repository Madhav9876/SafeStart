...
function buildGoogleUrl(appName: string) {
  ...
  const state = btoa(JSON.stringify({ origin: window.location.origin, appName, supabaseUrl, supabaseAnonKey: supabaseKey }));
...
export function signInWithGoogle(appName = 'AI Safety Maturity Quiz') {
  if (!supabase) {
    console.warn('[google-auth] Supabase is not configured');
    return;
  }
  ...
  const handler = async (event: MessageEvent) => {
    ...
    if (event.data.access_token && event.data.refresh_token) {
      ...
      const { error } = await client.auth.signInWithIdToken({ provider: 'google', token: event.data.id_token });
...
export async function handleGoogleRedirect() {
  ...
  try {
    ...
    console.warn('[google-auth] Could not close popup after sign-in');
  }
}
